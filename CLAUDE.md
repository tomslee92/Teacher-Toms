...
**Text-only home card behavior** (`HomeGridV2`): renders ONLY the single most recent note within a **14-day window** (`created_at`), text inline (action-blue left accent, right-aligned `— {teacher_name||Toms}` signature, `keep-all`, line breaks preserved). Marks `seen_at` fire-and-forget on render (not on tap). No sheet, no archive, no manual dismiss — notes expire quietly. `dismissed_at` is legacy-honored but no longer written. Verified end-to-end on Toms Lee (fresh renders, >14d hidden, delete restores, DB left untouched).
