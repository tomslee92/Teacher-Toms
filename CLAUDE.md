...
### Wavi Character + Listen/Shadowing Mode — scenarios (added 2026-06, commits 20b7d53→7ec177e)
A curated, scenario-based conversation mode students launch from the home screen, plus a character image on `WavyScreen`. Migrations: `supabase/migrations/20260606_wavi_scenarios_and_character.sql` (adds `phrase_bank.wavi_video_url`; tables `scenarios`, `scenario_lines`, `phrase_exposures`) and `20260609_scenario_assignments.sql` (the `scenario_assignments` join table + backfill).

**Curated library + shared assignment + home picker + rollout gate.** Scenarios are a reusable library, not one-off auto-launches. **Availability is decided by the `scenario_assignments` join table** (`scenario_id` + `student_id` | `group_id` | both-null = Everyone), so one scenario can target many students/groups; `scenarios.student_id`/`group_id` are now just the legacy initial scope (kept, backfilled into assignments, no longer read for availability). The student home (`HomeGridV2`) loads the card via assignments: assignment rows matching me ∪ my-group ∪ everyone → scenario ids → active `scenarios`. The **"추천 세션"** card lists them; tapping one sets `scenarioToLaunch` in AppInner → opens `WavyScreen` with `initialScenarioId` → Listen Mode. **Student-facing rollout is gated by `SCENARIOS_STUDENT_ENABLED` (constant ~line 163, currently `false`) — Toms-only via `scenariosVisible` name fallback in HomeGridV2.** This gate is deliberately INDEPENDENT of the V2 home flag (all students have V2, so V2 is not a safe gate). Flip the constant to `true` to roll out to all students.

**Teacher `ScenarioBuilder`** (student detail modal, rendered unconditionally — the view is teacher-only). The list is the **whole scenario library** (not per-student); each row has an **Assign** control with Everyone / per-group / per-student toggle chips that write `scenario_assignments` (needs `students`/`groups` passed from `StudentDetailView`). Create flow: title + context + initial scope + lines (speaker toggle, EN/KR, reorder) + "Generate with AI" (`geminiCallForFeedback` → JSON). Save writes `scenarios` + `scenario_lines` + one seed assignment, and shows a persistent ✓/✗ inline banner. **`{name}` token:** typing `{name}` in any line is replaced with the logged-in student's first name at playback + on screen (`fillName` in WavyScreen, case-insensitive).
