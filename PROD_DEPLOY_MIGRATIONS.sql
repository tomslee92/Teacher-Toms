...
-- ── 9) Wavi per-phrase attempt log (had no migration file; missing in prod) ──
...
CREATE INDEX IF NOT EXISTS wavy_phrase_attempts_student_idx ON public.wavy_phrase_attempts (student_id, session_date DESC);
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='wavy_phrase_attempts' AND rowsecurity)
    ...
    CREATE POLICY wavy_phrase_attempts_all ON public.wavy_phrase_attempts FOR ALL TO anon, authenticated USING (true) WITH CHECK (true);
  END IF;
END $$;
