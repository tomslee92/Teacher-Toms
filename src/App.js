...
function App() {
  ...
  if (screen === "teacher") return React.createElement(TeacherScreen, { homework, setHomework, students, setScreen });
        ...
        <div style={{ display: "flex", background: "rgba(255,255,255,0.6)", borderRadius: "16px", padding: "4px", marginBottom: "24px", border: "1px solid rgba(0,0,0,0.08)" }}>
          {[["student", "Student Login"], ["register", "Register"], ["teacher", "Teacher"]].map(function(item) {
            return React.createElement("button", {
              key: item[0],
              onClick: function() { setMode(item[0]); setError(""); },
              style: { flex: 1, padding: "10px", borderRadius: "12px", border: "none", background: mode === item[0] ? "#fff" : "transparent", color: mode === item[0] ? "#e8734a" : "#8a7e72", cursor: "pointer", fontSize: "13px", fontFamily: "Georgia, serif", boxShadow: mode === item[0] ? "0 2px 8px rgba(0,0,0,0.1)" : "none", transition: "all 0.2s" }
            }, item[1]);
          })}
        </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.8)", borderRadius: "20px", padding: "32px", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 8px 32px rgba(0,0,0,0.08)" }}>
          ...
          {mode === "teacher" && (
            <div>
              ...
              {error && <div style={{ color: "#e8734a", fontSize: "13px", marginBottom: "12px" }}>{error}</div>}
              <button onClick={handleTeacherLogin} style={{ width: "100%", padding: "14px", borderRadius: "12px", border: "none", background: "linear-gradient(135deg, #4a90d9, #7ab3e8)", color: "#fff", fontSize: "16px", cursor: "pointer", fontFamily: "Georgia, serif" }}>
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [{ role: "user", content: "You are Teacher Toms, a warm and encouraging English speaking coach for Korean learners. Your feedback is bilingual — give Korean explanations with English examples. Be like a supportive teacher.\n\nThe student just practiced this sentence: \"" + selectedSentence.text + "\"\nNative speaker tip: \"" + selectedSentence.tip + "\"\n\nGive feedback AS IF you heard them. Be warm, specific, and encouraging. Return ONLY JSON:\n{\n  \"score\": (1-10),\n  \"praise_ko\": \"(칭찬 — 한국어로, 1문장)\",\n  \"pronunciation_ko\": \"(발음 팁 — 한국어 설명)\",\n  \"pronunciation_en\": \"(English example of correct pronunciation)\",\n  \"grammar_ko\": \"(문법 팁 — 한국어 설명)\",\n  \"grammar_en\": \"(English example of correct grammar)\",\n  \"native_tip\": \"(How a native speaker says this naturally)\",\n  \"encouragement_ko\": \"(마지막 응원 한마디 — 한국어)\"\n}" }]
        })
      });
      ...
      const updated = {
        ...
        sessions: [newSession].concat((user.sessions || []).slice(0, 49))
      };
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "24px" }}>
          {[["homework", "📚 Homework"], ["progress", "📊 My Progress"]].map(function(item) {
            return React.createElement("button", {
              ...
              style: { padding: "10px 20px", borderRadius: "20px", border: "none", background: tab === item[0] ? "linear-gradient(135deg, #e8734a, #f0a86b)" : "rgba(255,255,255,0.7)", color: tab === item[0] ? "#fff" : "#8a7e72", cursor: "pointer", fontSize: "14px", fontFamily: "Georgia, serif", boxShadow: tab === item[0] ? "0 4px 12px rgba(232,115,74,0.3)" : "none" }
            }, item[1]);
          })}
        </div>
          ...
          <div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "20px" }}>
              {homework.map(function(hw) {
                return React.createElement("button", {
                  key: hw.id,
                  onClick: function() { setSelectedHW(hw); setSelectedSentence(null); setFeedback(null); setAudioBlob(null); },
                  style: { padding: "8px 16px", borderRadius: "20px", border: "1px solid " + (activeHW && activeHW.id === hw.id ? "#e8734a" : "rgba(0,0,0,0.12)"), background: activeHW && activeHW.id === hw.id ? "rgba(232,115,74,0.1)" : "rgba(255,255,255,0.7)", color: activeHW && activeHW.id === hw.id ? "#e8734a" : "#8a7e72", cursor: "pointer", fontSize: "13px", fontFamily: "Georgia, serif" }
                }, hw.week + " — " + hw.title);
              })}
            </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "24px" }}>
                  {activeHW.sentences.map(function(s, i) {
                    return React.createElement("button", {
                      key: s.id,
                      onClick: function() { setSelectedSentence(s); setFeedback(null); setAudioBlob(null); },
                      style: { textAlign: "left", padding: "16px 20px", borderRadius: "14px", border: "1px solid " + (selectedSentence && selectedSentence.id === s.id ? "#e8734a" : "rgba(0,0,0,0.08)"), background: selectedSentence && selectedSentence.id === s.id ? "rgba(232,115,74,0.08)" : "rgba(255,255,255,0.8)", cursor: "pointer", fontFamily: "Georgia, serif" }
                    },
                  ...
                  {isRecording && (
                    <div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", marginBottom: "16px" }}>
                        <div style={{ width: "10px", height: "10px", borderRadius: "50%", background: "#e8734a" }} />
                  ...
                  {audioBlob && !isRecording && (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "14px" }}>
                      ...
                      <div style={{ display: "flex", gap: "10px" }}>
                        <button onClick={getFeedback} disabled={isLoading} style={{ background: isLoading ? "rgba(74,144,217,0.2)" : "linear-gradient(135deg, #4a90d9, #7ab3e8)", border: "none", color: "#fff", padding: "14px 28px", borderRadius: "24px", cursor: isLoading ? "not-allowed" : "pointer", fontSize: "15px", fontFamily: "Georgia, serif" }}>
                  ...
                  <div style={{ marginTop: "24px", paddingTop: "24px", borderTop: "1px solid rgba(0,0,0,0.08)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <div style={{ padding: "6px 14px", borderRadius: "16px", background: feedback.score >= 8 ? "rgba(74,180,100,0.15)" : "rgba(240,168,107,0.15)", color: feedback.score >= 8 ? "#3a9a5c" : "#c07820", fontSize: "14px" }}>{feedback.score}/10</div>
                    ...
                    {[["✅ 잘한 점", feedback.praise_ko, null], ["🗣 발음", feedback.pronunciation_ko, feedback.pronunciation_en], ["📝 문법", feedback.grammar_ko, feedback.grammar_en], ["🌟 Native Tip", null, feedback.native_tip]].map(function(item) {
                      if (!item[1] && !item[2]) return null;
                      return React.createElement("div", { key: item[0], style: { marginBottom: "16px" } },
                        React.createElement("div", { style: { fontSize: "11px", letterSpacing: "1.5px", color: "#e8734a", textTransform: "uppercase", marginBottom: "6px" } }, item[0]),
                        item[1] && React.createElement("div", { style: { fontSize: "14px", color: "#4a4238", marginBottom: "4px", lineHeight: 1.6 } }, item[1]),
                        item[2] && React.createElement("div", { style: { fontSize: "14px", color: "#2a5a8a", fontStyle: "italic", lineHeight: 1.6 } }, '"' + item[2] + '"')
                      );
                    })}
                    <div style={{ marginTop: "16px", padding: "14px 16px", background: "rgba(232,115,74,0.06)", borderRadius: "12px", fontSize: "14px", color: "#8a5a3a", fontStyle: "italic" }}>💪 {feedback.encouragement_ko}</div>
                  </div>
                )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                {user.sessions.map(function(s, i) {
                  return React.createElement("div", { key: i, style: { background: "rgba(255,255,255,0.8)", borderRadius: "14px", padding: "16px 20px", border: "1px solid rgba(0,0,0,0.07)", display: "flex", justifyContent: "space-between", alignItems: "center" } },
                    ...
                    React.createElement("div", { style: { padding: "6px 12px", borderRadius: "12px", background: s.score >= 8 ? "rgba(74,180,100,0.12)" : "rgba(240,168,107,0.12)", color: s.score >= 8 ? "#3a9a5c" : "#c07820", fontSize: "14px" } }, s.score + "/10")
                  );
                })}
              </div>
            )}
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 1000,
          messages: [{ role: "user", content: "Generate 5 natural English practice sentences for Korean learners around the topic: \"" + generateTopic + "\". Return ONLY a JSON array with no extra text:\n[{\"text\": \"...\", \"tip\": \"one pronunciation/tone coaching tip in English\"}]" }]
        })
      });
      const data = await res.json();
      const text = data.content.map(function(i) { return i.text || ""; }).join("");
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "24px" }}>
          {[["assign", "📚 Assign Homework"], ["students", "👥 Students"]].map(function(item) {
            return React.createElement("button", {
              key: item[0],
              onClick: function() { setTab(item[0]); },
              style: { padding: "10px 20px", borderRadius: "20px", border: "none", background: tab === item[0] ? "linear-gradient(135deg, #4a90d9, #7ab3e8)" : "rgba(255,255,255,0.7)", color: tab === item[0] ? "#fff" : "#8a7e72", cursor: "pointer", fontSize: "14px", fontFamily: "Georgia, serif", boxShadow: tab === item[0] ? "0 4px 12px rgba(74,144,217,0.3)" : "none" }
            }, item[1]);
          })}
        </div>
