...
// ── Global Style ──────────────────────────────────────────────────────────────
const GlobalStyle = () => React.createElement(“style”, null, `@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap'); *{box-sizing:border-box;margin:0;padding:0;} body{font-family:${FONT};background:${C.bg};color:${C.text};} input,button,select,textarea{font-family:${FONT};} @keyframes spin{to{transform:rotate(360deg);}} @keyframes fadeIn{from{opacity:0;transform:translateY(6px);}to{opacity:1;transform:translateY(0);}} .fade-in{animation:fadeIn 0.2s ease;}`);
...
1. English/Latin characters: a-z A-Z 0-9 and standard punctuation
...
1. [Write another actual English example sentence]
   → [Write the actual Korean hangul translation]
...
messages: [{
role: “system”,
content: `You generate JSON arrays of English phrases for Korean learners. STRICT RULE: The "korean" and "context" fields must use ONLY Korean hangul characters (가-힣). NEVER use Chinese characters (漢字 like 食べ物, グルテン, etc.) or Japanese kana. If you want to write Japanese food terms, write them in Korean hangul only.`
}, {
role: “user”,
content: `Generate 8 useful English phrases for this situation: "${situation}" Korean translations and context MUST be in Korean HANGUL only — absolutely no Chinese or Japanese characters. Return ONLY valid JSON array: [{"english":"natural English phrase","korean":"Korean hangul translation ONLY","context":"Korean hangul context ONLY"}]${excludeList}`
}]
})
});
const d = await res.json();
...
messages: [{
role: “system”,
content: `You generate JSON arrays of English phrases for Korean learners. STRICT: "korean" and "context" fields must use ONLY Korean hangul (가-힣). No Chinese, no Japanese.`
}, {
role: “user”,
content: `Generate 5 English phrases for Korean learners about: "${topic}". All three fields required. "context" MUST be in Korean hangul only. Return ONLY valid JSON array: [{"english":"natural English phrase","korean":"Korean hangul translation","context":"Korean hangul explanation of when to use this phrase"}]`
}]
})
});
const d = await res.json();
...
React.createElement(“style”, null, `@keyframes confettiFall { 0% { transform: translateY(-20px) rotate(0deg); opacity: 1; } 100% { transform: translateY(100vh) rotate(720deg); opacity: 0; } }`),
