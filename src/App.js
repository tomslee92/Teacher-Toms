...
async function speak(text, speed = null) {
  ...
  // Check cache first — same text always plays same audio
  ...
  try {
    ...
    ttsCache.set(text.trim(), url); // cache for consistent replay
    ...
    <div style={{ minHeight: "100vh", background: C.bg, paddingBottom: "72px" }}>
      ...
      <div style={{ background: C.bg, borderBottom: `1px solid ${C.border}`, padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            {/* Font size — single Aa button with dropdown */}
...
function FreeTalkTab({ user, isPreview, onPracticed }) {
  ...
  return (
    <div>
      <div style={{ background: C.bgDark, borderRadius: "20px", padding: "24px", marginBottom: "20px" }}>
        <div style={{ fontSize: "11px", fontWeight: "600", color: "rgba(255,255,255,0.4)", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "6px" }}>Open Conversation</div>
        <div style={{ fontSize: "22px", fontWeight: "900", color: "#fff", letterSpacing: "-0.3px", marginBottom: "16px" }}>💬 Free Talk</div>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          {tabs.map(([m, label]) =>
            React.createElement("button", { key: m, onClick: () => { setMode(m); setFeedback(null); setTranscript(""); },
              style: { padding: "7px 16px", borderRadius: "100px", border: "none", background: mode === m ? "#fff" : "rgba(255,255,255,0.12)", color: mode === m ? C.text : "rgba(255,255,255,0.7)", fontSize: "13px", fontWeight: mode === m ? "700" : "500", cursor: "pointer", fontFamily: FONT, transition: "all 0.15s" } }, label)
          )}
        </div>
      </div> <div style={{ display: "flex", borderBottom: `1px solid ${C.border}`, marginBottom: "20px", overflowX: "auto", overflowY: "hidden", scrollbarWidth: "none", msOverflowStyle: "none" }}>
        {tabs.map(([m, label]) =>
  ...
  return (
    <React.Fragment>
      ...
      {open && (
        <div style={{
          position: "fixed",
          bottom: window.innerWidth < 600 ? "148px" : "96px",
          right: window.innerWidth < 600 ? "12px" : "16px",
          left: window.innerWidth < 600 ? "12px" : "auto",
          top: window.innerWidth < 600 ? "60px" : "auto",
          width: window.innerWidth < 600 ? "calc(100vw - 24px)" : "min(380px, calc(100vw - 32px))",
          maxHeight: window.innerWidth < 600 ? "calc(100vh - 220px)" : "min(520px, calc(100vh - 110px))",
          height: window.innerWidth < 600 ? "calc(100vh - 220px)" : "min(520px, calc(100vh - 110px))",
          background: C.bg,
          borderRadius: "16px",
...
function QodEntryScreen({ user, group, onEnter }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column" }}>
      ...
      {/* Streak badge — top of screen, hero number */}
      <div style={{ padding: "20px 24px 0", display: "flex", justifyContent: "center", animation: "qodEntryFade 0.4s ease both" }}>
        {streak > 0 ? (
          <div style={{ display: "inline-flex", alignItems: "center", gap: "16px", background: C.bgSoft, borderRadius: "20px", padding: "18px 28px", border: `1px solid ${C.border}` }}>
            <span style={{ fontSize: "36px", animation: "streakFire 2.5s ease-in-out infinite", display: "inline-block", lineHeight: 1 }}>🔥</span>
            <div>
              <div style={{ display: "flex", alignItems: "baseline", gap: "6px", marginBottom: "3px" }}>
                <span style={{ fontSize: "44px", fontWeight: "900", color: C.text, letterSpacing: "-2px", lineHeight: 1 }}>{streak}</span>
                <span style={{ fontSize: "18px", fontWeight: "700", color: C.text, letterSpacing: "-0.3px" }}>day{streak !== 1 ? "s" : ""}</span>
              </div>
              <div style={{ fontSize: "12px", color: C.textLight, letterSpacing: "0.2px" }}>Keep the streak going 💪</div>
            </div>
          </div>
        ) : (
...
function FontSizeToggle({ fontSize, setFontSize }) {
  ...
  const sizes = [
    ...
    { id: "xlarge",  label: "X-Large", sample: "20px" },
  ];
  ...
  return React.createElement("div", { ref, style: { position: "relative" } },
    ...
    React.createElement("button", {
      ...
      style: {
        ...
        background: fontSize !== "default" ? C.text : C.bgSoft,
        border: `1px solid ${fontSize !== "default" ? C.text : C.border}`,
        color: fontSize !== "default" ? "#fff" : C.textMid,
    ...
    open && React.createElement("div", {
      ...
      ...sizes.map(s =>
        React.createElement("button", {
          key: s.id,
          onClick: () => { setFontSize(s.id); setOpen(false); },
          style: {
            width: "100%", padding: "9px 12px", borderRadius: "8px",
            background: fontSize === s.id ? C.bgSoft : "transparent",
            border: "none", cursor: "pointer", fontFamily: FONT,
            display: "flex", alignItems: "center", justifyContent: "space-between",
            transition: "background 0.1s",
          }
        },
          React.createElement("span", { style: { fontSize: "13px", fontWeight: fontSize === s.id ? "700" : "400", color: C.text } }, s.label),
          fontSize === s.id && React.createElement("span", { style: { color: C.text, fontSize: "14px" } }, "✓")
        )
      )
    )
  );
}
      {/* Header card matching home grid style */}
      <div style={{ background: C.bgDark, borderRadius: "20px", padding: "24px", marginBottom: "20px" }}>
        <div style={{ fontSize: "11px", fontWeight: "600", color: "rgba(255,255,255,0.4)", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "6px" }}>Open Conversation</div>
        <div style={{ fontSize: "22px", fontWeight: "900", color: "#fff", letterSpacing: "-0.3px", marginBottom: "16px" }}>💬 Free Talk</div>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
      ...
      </div> { useState, useEffect, useRef, useCallback } from "react";
