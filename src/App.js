        ...
        {streak > 0 ? (
          <div style={{ display: "inline-flex", alignItems: "center", gap: "16px", background: C.bgSoft, borderRadius: "20px", padding: "18px 28px", border: `1px solid ${C.border}` }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "12px", color: C.textLight }}>Keep the streak going 💪</div>
            </div>
          </div>
        ) : (
...
function HomeGrid({ user, group, isPreview, onNavigate, streak }) {
  ...
  return (
    <div>
      ...
      {streak > 0 && (
        <div style={{ background: C.bgDark, borderRadius: "14px", padding: "14px 18px", marginBottom: "16px", display: "flex", alignItems: "center", justifyContent: "space-between", color: "#fff", animation: "cardReveal 0.3s ease both" }}>
          <div>
...
function TeacherCommentDisplay({ responseId }) {
  ...
  useEffect(() => {
    db.get("qod_comments", `response_id=eq.${responseId}&limit=1`).then(r => { if (r.length > 0) setComment(r[0]); }).catch(() => {});
  }, [responseId]);
  if (!comment) return null;
  return React.createElement("div", { style: { background: C.goldBg, border: `1px solid ${C.goldBorder}`, borderRadius: "10px", padding: "12px", marginTop: "8px" } },
    React.createElement("div", { style: { fontSize: "10px", fontWeight: "700", color: C.gold, letterSpacing: "2px", textTransform: "uppercase", marginBottom: "6px" } }, "👨‍🏫 Teacher Toms' Feedback"),
    comment.teacher_text && React.createElement("div", { style: { fontSize: "13px", color: C.text, lineHeight: 1.65, marginBottom: comment.audio_url ? "8px" : "0" } }, comment.teacher_text),
